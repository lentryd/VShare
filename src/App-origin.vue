<template>
  ...
  <NavBar v-if="$device.isMobile" :show="!hideBar" />
</template>
...
export default defineComponent({
  ...
  computed: {
    ...
    isAnonymous() {
      return this.$auth.isAnonymous ?? true;
    },

    hideBar() {
      return this.$route.meta.hideBar ?? false;
    },
    transition() {
      return !this.$route.meta.inverseTransition
        ? "show-route"
        : "show-route-inverse";
    },
    onlyAnonymous() {
      return this.$route.meta.onlyAnonymous ?? false;
    },
  },
  ...
  watch: {
    loading(v) {
      ...
      if (el) el.dataset.hide = (!v).toString();

      setTimeout(this.fromAuth, 0);
    },
    isAnonymous() {
      this.fromAuth();
    },
  ...
  methods: {
    fromAuth() {
      if (this.isAnonymous || !this.onlyAnonymous) return;
